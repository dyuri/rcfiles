...
class BtwOverlay extends Container implements Focusable {
	...
	handleInput(data: string): void {
		...
		// Scroll up with Page Up or arrow up
		if (this.keybindings.matches(data, "tui.select.selectPrevious") || data === "\u001b[5~") {
			this.scrollOffset = Math.min(this.scrollOffset + 5, this.getMaxScroll());
			this.tui.requestRender();
			return;
		}

		// Scroll down with Page Down or arrow down
		if (this.keybindings.matches(data, "tui.select.selectNext") || data === "\u001b[6~") {
	...
	private getMaxScroll(): number {
		// Calculate how much we can scroll up from the bottom
		return Math.max(0, this.getTranscript(80, this.theme).length - 6);
	}
	...
	override render(width: number): string[] {
		...
		// Apply scroll offset: show from (total - transcriptHeight - scrollOffset) to (total - scrollOffset)
		...
		const scrollHint = this.scrollOffset > 0 ? " · PgUp/PgDn to scroll" : "";
		lines.push(this.frameLine(this.theme.fg("dim", `Enter submit · Esc close${scrollHint}`), innerWidth));
